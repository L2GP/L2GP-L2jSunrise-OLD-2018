...
public abstract class L2Character extends L2Object implements ISkillsHolder
{
	...
	public void doAttack(L2Character target)
	{
		...
		// L2GamePlay: this will fix broken attack animations from mobs
		if (isAttackable())
		{
		...
		// L2GamePlay: Just in case npc attack end time is not set
		if (isNpc() || isSummon())
		{
	...
	private void beginCast(L2Skill skill, boolean simultaneously)
	{
		...
		// L2GamePlay: herb effects should not remove skills on action example: removedOnAnyActionExceptMove
		if (!skill.isHerb())
		{
			stopEffectsOnAction();
		}
		...
		switch (skill.getTargetType())
		{
			...
			default:
				if (targets.length == 0)
				{
					...
					// L2GamePlay setting back previous intention
					else if (isSummon())
					{
	...
	private void beginCast(L2Skill skill, boolean simultaneously, L2Character target, L2Object[] targets)
	{
		...
		// L2GamePlay: shadow mana consumption must be checked before skill cast start
		if (isPlayable())
		{
	...
	{
		// L2GamePlay Deny the whole process if actor is casting.
		if (isCastingNow())
		{
			return;
		}
		...
		// L2GamePlay: auto attack must start only if hit didn't miss
		...
		if (!miss && (damage > 0))
		{
			...
			if (!isBow && !target.isInvul()) // Do not reflect if weapon is of type bow or target is invulnerable
			{
				...
				{
					// L2GamePlay this will fix max reflect to 100%
			...
			if (_chanceSkills != null)
			{
				...
				if (reflectedDamage > 0)
				{
					// TODO Test L2GamePlay fix: this will fix wrong skills triggers from reflect damage
	...
	public void onMagicLaunchedTimer(MagicUseTask mut)
	{
		...
		L2Object[] targets;
		// L2GamePlay: this will fix target checks when skill finish cast
				...
				for (L2Object target : targets)
				{
					...
					if (target instanceof L2Character)
					{
						...
						else
						{
							...
							// L2GamePlay: Fix Casting non offensive skill on pet with pvp flag set or with karma
	...
	public void makeTriggerCast(L2Skill skill, L2Character target)
	{
		try
		{
			// L2GamePlay if target is already casting or dead must return
	...
	// L2GamePlay addon some mobs must have higher watch-forget distance
